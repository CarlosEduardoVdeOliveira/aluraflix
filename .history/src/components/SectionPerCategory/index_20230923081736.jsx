...
export default function SectionPerCategory() {
  ...
  function fiterCategory(category){
    const videoCategory = videos.map(item => item.category)
    const uniqueCategory = videos.some(item => item.category === category)
    setCategory([uniqueCategory])
    console.log(category);
  }
  fiterCategory('Front End')
  return (
  <>
      {
        videos.map(video =>{
