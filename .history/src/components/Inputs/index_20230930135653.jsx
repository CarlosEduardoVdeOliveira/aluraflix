...
export const TextArea = ({placeholder, value}) =>{
  return(
    <textarea value={value} className={styles.textarea} placeholder={placeholder}></textarea>
  )
}
...
export const Select = ({categories, value}) => {
  return(
    <>
      <select value={value} className={styles.input} name={categories}>
      {
          categories.map(category => (
            <option key={value} value={category.name}>{category.name}</option>
          ))
        }
      </select>
    </>
