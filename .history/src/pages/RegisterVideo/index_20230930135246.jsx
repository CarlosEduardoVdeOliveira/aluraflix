...
export const RegisterVideo = () => {
  ...
  return (
    <form onSubmit={() =>registerVideo} className={styles.container}>
      <h2>Novo vídeo</h2>
      <Input value={title} onChange={(event)=> console.log(setTitle(event.target.value))} placeholder="Título" type="text" />
      <Input value={videoLink} onChange={(event)=> console.log(setVideoLink(event.target.value))} placeholder="Link do vídeo" type="text" />
      <Input value={imageLink} onChange={(event)=> console.log(setImageLink(event.target.value))} placeholder="Link da imagem do vídeo" type="text" />
      <Select value={category} onChange={(event)=> console.log(setCategory(event.target.value))} categories={categories} />
