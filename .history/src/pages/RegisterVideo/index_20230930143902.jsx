...
export const RegisterVideo = () => {
  ...
  function registerVideo(event){
    ...
    console.log({title, videoLink, imageLink, category, description, codigo});
  }
  return (
    <form onSubmit={()=>registerVideo} className={styles.container}>
      ...
      />
      <TextArea value={description} onChange={(event)=> (console.log(event.target.value))} placeholder="Descrição"></TextArea>
      <Input value={codigo} onChange={(event)=> (console.log(event.target.value))} placeholder="Código de segurança" type="text" />
